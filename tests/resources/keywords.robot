...
Enter Vehicle Data for Automobile
    ...
    Select Options By    id=model    text    Scooter
    Fill Text    id=engineperformance    110
    Fill Text    id=dateofmanufacture    06/12/1980
    Fill Text    id=cylindercapacity    55
    Check Checkbox    *css=label >> id=righthanddriveyes
    Select Options By    id=numberofseatsmotorcycle    text    2
    Select Options By    id=numberofseats    text    5
    Fill Text    id=payload    1
    Fill Text    id=totalweight    120
...
Enter Insurant Data
    [Arguments]    ${firstname}=David    ${lastname}=John
    Fill Text    id=firstname    David
    Fill Text    id=lastname    John
